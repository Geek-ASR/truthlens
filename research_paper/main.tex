...
\item \textbf{Diagnose why claim extraction almost never populates a
  ...
  the clearest concrete next step from this pass's own findings -- not
  yet attempted; two live hypotheses (prompt under-elicitation vs.\
  genuinely unquotable paraphrases) remain undistinguished.
\item \textbf{An input-signal consistency check} for claim extraction
  (does a post's caption agree with its transcript/OCR content) and a
  \textbf{multilingual-extraction audit} (does non-Latin-script content
  get dropped disproportionately) -- both newly named, neither
  investigated further, in Section~\ref{sec:foundationadversarial}.
