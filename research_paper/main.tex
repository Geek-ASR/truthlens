...
baseline's balanced accuracy. The binding constraint is localised
precisely -- claim extraction, not evidence retrieval (0\%
research-failure) or verdict reasoning: 65.3\% of real posts yield no
checkable claim to the local model, and the \texttt{MISLEADING} class is
never once recovered (0/7). Strengthening that configuration, and a
...
with claim extraction -- not retrieval or verdict reasoning --
identified as the component that decides whether a post gets checked
at all. We report every
...
\item \textbf{\textsc{TruthLens-202}}: a 202-item held-out benchmark of
  ...
  composition and limitations -- a single LLM-assisted reviewer,
  $\sim$88\% single-source ground truth, and three domain-driven skews --
...
\midrule
Ground-truth label & \texttt{FALSE} 167 (82.7\%), \texttt{MISLEADING} 30, \texttt{TRUE} 5 \\
Source platform & X/Twitter 161 (79.7\%), Instagram 25, Facebook 16 \\
Language & English 146, Hindi 52, Bengali 3, Arabic 1 \\
Claim type & \texttt{provenance} 161 (79.7\%), \texttt{event} 12, \\
 & \texttt{misleading\_context} 12, \texttt{quote} 11, other 6 \\
Media & video 200, photo 2 \\
Fact-checker & Alt News 170, Lead Stories 14, BOOM 4, \\
 (\texttt{validation}) & Check Your Fact 2, others 3 \\
Modality present & caption 192/193, audio 163/193, OCR 91/193 \\
 (\texttt{validation}) & (visual-context analysis deferred on 115) \\
...
measured against this split. \textbf{(2)~Single, LLM-assisted reviewer.}
The claim phrasing, label mapping, and cross-post/reject decisions were
produced by one reviewer working with LLM assistance, not by two
independent human annotators, so no inter-annotator agreement statistic
is available (and none is fabricated). The verdict itself always traces
to an independent professional fact-check via the recorded quote, which
bounds the risk, but the phrasing and mapping are unverified by a second
human -- a real circularity concern given that an LLM-based system is the
intended evaluee. A second-annotator pass is the top open item
...
False-abstention rate (all items) & 155 / 193 \quad(80.3\%) \\
\bottomrule
\end{tabular}
\end{table}
...
pattern (80\% of the corpus is provenance-type). Inspection of the 11
resolved items where a \texttt{FALSE}/\texttt{MISLEADING} claim received
an affirmative (\texttt{TRUE}/\texttt{MOSTLY\_TRUE}) verdict shows the
same root cause as the abstentions: the ``claim'' the model extracted
was a fragment of on-screen dialogue (``These scopes are there!'';
``Where did you go?''; a recital of the US Pledge of Allegiance), not
the post's actual assertion. The pipeline then correctly verified a
trivially-true fragment -- a claim-\emph{coverage} failure
(Section~\ref{sec:multimodalanalysis}), not a verdict-reasoning failure.
...
3B model, \emph{claim extraction} -- not retrieval, not verdict
reasoning -- is the component that determines whether short-form
political misinformation gets checked at all, and it fails on roughly
two-thirds of real posts; (ii) the deterministic safeguards hold up
...
source type \emph{under the Instagram-only constraint} in any bounded
timeframe -- a structural property of where this specific misinformation
pattern actually gets posted, not a tooling or effort gap: five sources
were checked, two archives fully exhausted on a second, broadened pass,
and every judge-output anomaly found was traced to a specific, fixed
cause rather than shrugged off as noise. Full detail, including every
individual promoted/rejected candidate's disclosed reasoning:
\texttt{research/MASS\_SOURCING\_V2.md}.

\textbf{This finding was subsequently acted on.} Because the ceiling
was a property of the platform restriction rather than of the sourcing
method, a later pass relaxed that restriction -- admitting posts from
X/Twitter, Facebook, and Instagram -- and added a review-gated
annotation step in which a single reviewer adjudicates each screened
candidate against the full professional fact-check article. This grew
the \texttt{validation} split from 13 to 193 items (202 including the
frozen \texttt{dev} set), yielding the \textsc{TruthLens-202} corpus
described in Section~\ref{sec:truthlens202}. That corpus is then
evaluated end-to-end once, in a \$0 local-only configuration
(Section~\ref{sec:tl202eval}), which surfaces its own finding: on real,
uncurated short-form posts the local 3B model fails to extract a
checkable claim from 65.3\% of them, making claim extraction -- not
retrieval or verdict reasoning -- the binding constraint. Its
construction carries its own disclosed limitations (single LLM-assisted
reviewer; $\sim$88\% single-source ground truth; three domain-driven
skews), all stated in Section~\ref{sec:truthlens202}.
...
\label{sec:threats}
Twelve threats, stated here at the density needed to be checkable; full
reasoning for the first ten: Appendix~\ref{sec:appendixthreats}. The last
two concern the \textsc{TruthLens-202} corpus and its local-only
evaluation and are detailed in Sections~\ref{sec:truthlens202} and
\ref{sec:tl202eval}.
...
\item \textbf{Ground truth is sourced from a small set of professional
  ...
  check against.
\item \textbf{\textsc{TruthLens-202}'s claim/label annotations are a
  single LLM-assisted reviewer's work, not verified by a second human}
  (Section~\ref{sec:truthlens202}). Verdicts trace to an independent
  professional fact-check via a recorded verbatim quote, which bounds
  the risk, but the claim phrasing and label mapping are unaudited --
...
\begin{enumerate}
\item \textbf{Scale the held-out benchmark past $n=9$} -- \emph{addressed
  as a corpus and evaluated once; stronger configurations now the open
  work}. The benchmark grew to 15
  (Section~\ref{sec:foundationphase}), then to 22 under an
  Instagram-only constraint (Section~\ref{sec:massSourcing}) at a
  measured yield of roughly one item per 900 candidates. Relaxing that
  constraint to all short-form platforms and adding a review-gated
  annotation step grew the \texttt{validation} split to 193 items --
  \textsc{TruthLens-202} (Section~\ref{sec:truthlens202}) -- which was
  then run end-to-end in the local-only configuration
  (Section~\ref{sec:tl202eval}: 29.9\% bucketed accuracy on the 34.7\%
  of items that resolve; claim extraction the binding constraint). What
  remains: (a)~re-run the same held-out split under a larger local model
  (\texttt{llama3.1:8b}/\texttt{mistral}) and a metered cloud-escalation
  tier, reporting the three as an ablation of capability vs.\ cost;
  (b)~a vision-first claim-extraction path for provenance-type posts,
  where the visual \emph{is} the claim and text-only extraction (shown
  here to fail on 65\% of items) cannot succeed; (c)~a second
  independent annotator pass (item~8); (d)~broaden the ground-truth base
  beyond the current $\sim$88\% single-source concentration.
...
\item \textbf{A second, independent annotator}, for a real
  inter-annotator agreement statistic on Tier-2 items, the draft
  judgments underlying Sections~\ref{sec:evidencevalidation}
  and~\ref{sec:multimodalanalysis}, and -- now the largest instance --
  the 165 review-gated claim/label annotations in
  \textsc{TruthLens-202}'s \texttt{validation} split, whose phrasing and
  label mapping are currently the work of a single LLM-assisted reviewer
  (Section~\ref{sec:truthlens202}). This is the top open item before the
  local-only result (Section~\ref{sec:tl202eval}) can be treated as a
  validated headline rather than a floor.
...
Acting on that session's own structural finding -- that the yield
ceiling was a property of the Instagram-only constraint, not of the
sourcing method -- a subsequent pass relaxed the platform scope to all
short-form political posts and added a review-gated annotation step,
growing the \texttt{validation} split from 13 to 193 items:
\textsc{TruthLens-202} (Section~\ref{sec:truthlens202}). This makes a
benchmark at the scale this program set out to build finally available,
and we run the full pipeline over all 193 of its \texttt{validation}
items once, in a \$0 local-only configuration
(Section~\ref{sec:tl202eval}). That result is reported as a floor, with
its limitations stated rather than softened: 34.7\% of posts reach a
verdict at all, bucketed accuracy on those is 29.9\% (below an
always-\texttt{FALSE} baseline on balanced accuracy), the annotations
are one LLM-assisted reviewer's work pending a second annotator, and the
corpus carries a single-source majority and three domain-driven skews.
Its one firm, transferable finding is diagnostic: with a local 3B model,
\emph{claim extraction} -- not retrieval (0\% infrastructure failure)
and not verdict reasoning -- is what determines whether short-form
political misinformation gets checked, and it fails on roughly
two-thirds of real posts.

Work on this program is paused at this point. What is left open, stated
as concretely as everything reported above: the \texttt{validation}
split has been evaluated only in the local-only configuration, and the
larger-local-model, vision-first, and metered-cloud-escalation
configurations -- reported together as a capability-vs.-cost ablation --
plus a second independent annotator and a broader ground-truth base are
the immediate next work
(Section~\ref{sec:futurework}); RQ5 and RQ6 remain deferred for the same
pre-registered reason as before; the claim-extraction coverage gap, the
cross-post attribution problem, and the verdict-stage
reliability-under-contradiction finding are each diagnosed but not
resolved; and the publishing pipeline's real-account connection, and
everything downstream of it (a first live post, its own review-queue
usage in practice, and any resulting engagement or correction workflow),
is unattempted. None of these are hidden or understated in the sections
above; collected here only so a resumption of this work starts from an
accurate list rather than a reconstruction of one.
