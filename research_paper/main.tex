...
\item \textbf{Complete the full end-to-end adversarial suite} (Step 18's
  ...
  unbuilt.
\item \textbf{A deterministic reliability-weighted verdict check.} A
  measured, reproducible failure (Section~\ref{sec:secondpass}): verdict
  -label selection does not reliably favor a high-reliability source
  over a low-reliability, uncited one under direct contradiction, and a
  targeted prompt fix did not resolve it (0/14 real trials correct,
  before and after). The likely real fix -- comparing cited-evidence
  reliability against verdict direction as an 8th deterministic
  validator check, mirroring Checks 6/7's successful pattern -- is named
  but not built, pending its own integration evaluation.
...
itself, all but four with a deterministic check and a regression test
...
\item \textbf{Verdict-label selection does not reliably weight source
  ...
  silently dropped, with a deterministic validator check (comparing
  cited-evidence reliability against verdict direction) named as the
  likely real fix and left for a dedicated future pass rather than
  added without its own evaluation.
\end{itemize}
