...
65.3\% of real posts, making claim extraction -- not retrieval or
verdict reasoning -- the binding constraint. Its construction carries
disclosed limitations (single LLM-assisted reviewer; $\sim$88\%
single-source ground truth; three domain-driven skews), all stated in
...
\item \textbf{A second, independent annotator} for the 165 review-gated
  \textsc{TruthLens-202} claim/label annotations (currently one
  LLM-assisted reviewer) and the Tier-2 draft judgments underlying
...
annotations are one LLM-assisted reviewer's work pending a second
annotator, and the corpus carries a single-source majority and three
domain-driven skews. Its one firm, transferable finding is diagnostic:
with a local 3B model, \emph{claim extraction} -- not retrieval (0\%
infrastructure failure) and not verdict reasoning -- determines whether
short-form political misinformation gets checked at all, and it fails on
roughly two-thirds of real posts. Work is paused here; the immediate
