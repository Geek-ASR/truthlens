...
pass/fail reading would suggest. Every number in this paper is reported with its exact sample size
...
(EXP-009 through EXP-020), and negative or inconclusive results are kept
...
\item \textbf{Video-provenance verification.} \emph{Partially addressed}:
  a perceptual-hashing first-pass filter is built and validated against
  real video frames (Section~\ref{sec:foundationphase}), but the full
  3-stage design (claim-embedding clustering, Gemini escalation for
  ambiguous pairs) and any live pipeline integration remain undone.
...
\item \textbf{Complete the full end-to-end adversarial suite} (Step 18's
  20 categories, 100+ cases): this pass delivered a bounded 6-case slice
  of claim-extraction-only robustness; categories needing real
