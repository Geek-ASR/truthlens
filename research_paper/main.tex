...
and are explicit that \emph{no reel-level accuracy on
\textsc{TruthLens-202} is reported here} -- running the full pipeline over
it, and a second independent annotator pass, are the immediate next steps.
...
candidates screened. \textsc{TruthLens-202} is presented as an
evaluation artifact with its construction and limitations documented in
full (Section~\ref{sec:truthlens202}); it has not yet been run through
the pipeline, and no accuracy on it is reported here. We report every
...
\item \textbf{\textsc{TruthLens-202}}: a 202-item held-out benchmark of
  ...
  are reported; end-to-end evaluation on it is explicitly left as future
  work, so it is a contributed artifact, not a contributed result.
\end{enumerate}
...
made feasible; it is a corpus for future end-to-end evaluation, and no
reel-level accuracy on it is reported in this paper.
...
future end-to-end evaluation. It comprises 202 real short-form political
posts: the 9 comparison items form its \texttt{dev} split, and 193 newly
collected items form a \texttt{validation} split that no result in this
paper touches. We describe how it was built, its composition, and its
limitations in full, because the way an evaluation set is constructed is
itself a claim that should be checkable.
...
\textbf{(1)~Not yet evaluated.} No item in the \texttt{validation} split
has been run through the full TruthLens pipeline; this paper reports the
corpus, not a result on it. \textbf{(2)~Single, LLM-assisted reviewer.}
...
described in Section~\ref{sec:truthlens202}. The corpus is built but not
yet evaluated end-to-end, and its construction carries its own disclosed
limitations (single LLM-assisted reviewer; $\sim$88\% single-source
ground truth; three domain-driven skews), all stated in
...
two concern the \textsc{TruthLens-202} corpus and are detailed in
Section~\ref{sec:truthlens202}.
...
\item \textbf{\textsc{TruthLens-202}'s claim/label annotations are a
  ...
  evaluee. No result on that split is reported until this is addressed.
\item \textbf{\textsc{TruthLens-202} is $\sim$88\% single-source (Alt
  News) and carries three domain-driven skews} (83\% \texttt{FALSE},
  80\% X-sourced, 80\% \texttt{provenance}-type); any end-to-end
  evaluation on it must use balanced, per-class metrics and treat the
  source concentration as a coverage limitation, not a representative
  sample of the fact-checking ecosystem.
\end{itemize}
...
\item \textbf{Scale the held-out benchmark past $n=9$} -- \emph{addressed
  as a corpus; end-to-end evaluation now the open work}. The benchmark
  grew to 15 (Section~\ref{sec:foundationphase}), then to 22 under an
  ...
  \textsc{TruthLens-202} (Section~\ref{sec:truthlens202}). What remains:
  (a)~run the full TruthLens pipeline over all 193 \texttt{validation}
  items and report reel-level accuracy with per-class breakdowns
  (balanced, given the 83\% \texttt{FALSE} skew); (b)~a second
  independent annotator pass (item~8); (c)~a backfill of the deferred
  vision-context stage on 115 items; (d)~broaden the ground-truth base
...
\item \textbf{A second, independent annotator}, for a real
  ...
  (Section~\ref{sec:truthlens202}). This is the top open item before any
  result on that split can be reported.
...
It is deliberately reported as a corpus, not a result: no item in the
\texttt{validation} split has been run through the pipeline, its
claim/label annotations are one LLM-assisted reviewer's work pending a
second annotator, $\sim$88\% of its ground truth is from a single
organization, and its label/platform/claim-type distributions carry
three domain-driven skews that any evaluation on it must handle with
per-class metrics. Every one of those limitations is stated in full in
Section~\ref{sec:truthlens202}, not softened here.
...
split exists at $n=193$ but has not been evaluated end-to-end, and doing
so -- with a second independent annotator, a balanced per-class metric,
a vision-context backfill on 115 items, and a broader ground-truth base
than the current single-source majority -- is the immediate next work
